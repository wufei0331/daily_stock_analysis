...
def _format_zh(payload: Dict[str, Any]) -> str:
    ...
    # ==========================================
    lines.append("")
    lines.append("## 🚨 AI 深度策略切片 (强制执行规则) 🚨")
    lines.append("在输出标准决策仪表盘之前，你**必须**严格运用以下 5 个原版实战策略对该股票进行深度诊断。如果符合条件，必须在报告中明确指出：")
    
    # 1. 缩量回踩
    lines.append("1. **缩量回踩 (Shrink Pullback)**：")
    lines.append("   - 前提：处于上升趋势 (MA5 > MA10 > MA20)。")
    lines.append("   - 信号：价格回踩至 MA5 或 MA10 附近，且回调期间成交量严重萎缩（< 5日均量的70%）。")
    lines.append("   - 结论：若守住支撑，提示为极佳买入区间（MA20止损）。")
    
    # 2. 底部放量
    lines.append("2. **底部放量 (Bottom Volume)**：")
    lines.append("   - 前提：长期持续下跌，跌幅 > 15%。")
    lines.append("   - 信号：极度缩量后，当日成交量突然 > 5日均量的 3 倍，且K线收阳企稳（最好有长下影线）。")
    lines.append("   - 结论：提示潜在趋势反转，但属于左侧交易，建议小仓位并严格以前低止损。")
    
    # 3. 箱体震荡
    lines.append("3. **箱体震荡 (Box Oscillation)**：")
    lines.append("   - 信号：识别近60日内多次触碰未破的顶部（阻力）和底部（支撑）。计算当前价格处于箱底(距离支撑<=5%)、箱中(观望)还是箱顶(距离阻力<=5%)。")
    lines.append("   - 结论：箱底企稳看多，箱顶滞涨看空，若放量突破箱体则转为多头趋势。")
    
    # 4. 波浪理论
    lines.append("4. **波浪理论 (Wave Theory)**：")
    lines.append("   - 信号：识别当前处于推动浪(1-3-5)还是调整浪(A-B-C)。重点排查是否处于【第2浪回调企稳(黄金坑)】或【第3浪放量突破】。")
    lines.append("   - 结论：若处于B浪反弹提示陷阱风险；若处于第5浪末端提示顶背离风险。")
    
    # 5. 缠论
    lines.append("5. **缠论 (Chan Theory)**：")
    lines.append("   - 信号：识别中枢震荡或趋势段。寻找最高优先级信号——背驰。")
    lines.append("   - 结论：若出现【底背驰】(创新低但MACD绿柱缩小)提示一买信号；若突破中枢回踩不破提示三买；若出现【顶背驰】坚决提示卖出。")
    
    lines.append("【输出绝对指令】：你必须在最终报告中新增【🧠 核心策略诊断】专属段落，直接套用上述 5 个策略中**最符合该股票当前走势的 1 到 2 个策略**进行点评，不要罗列不相关的策略，给出明确的支撑阻力位与买卖倾向！")
    # ==========================================
    return "\n".join(lines) + "\n"
